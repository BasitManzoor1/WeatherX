...
class DataStore(private val context: Context) {
    ...
    // Theme
    val themeMode = stringPreferencesKey("theme_mode") // ON, OFF, SYSTEM
    
    // Notifications
    val notifyRain = booleanPreferencesKey("notify_rain")
    val notifySnow = booleanPreferencesKey("notify_snow")
    val notifySevere = booleanPreferencesKey("notify_severe")
    val notifyHeatwave = booleanPreferencesKey("notify_heatwave")

    suspend fun <T> write(key: Preferences.Key<T>, value: T) {
        context.dataStore.edit { it[key] = value }
    }

    val cityFlow: Flow<String> = context.dataStore.data.map { it[city] ?: "Athens" }
    val useGpsFlow: Flow<Boolean> = context.dataStore.data.map { it[useGps] ?: false }
    
    val themeModeFlow: Flow<String> = context.dataStore.data.map { it[themeMode] ?: "SYSTEM" }
    
    val notifyRainFlow: Flow<Boolean> = context.dataStore.data.map { it[notifyRain] ?: true }
    val notifySnowFlow: Flow<Boolean> = context.dataStore.data.map { it[notifySnow] ?: true }
    val notifySevereFlow: Flow<Boolean> = context.dataStore.data.map { it[notifySevere] ?: true }
    val notifyHeatwaveFlow: Flow<Boolean> = context.dataStore.data.map { it[notifyHeatwave] ?: true }

    suspend fun writeCity(value: String) = write(city, value)
}
